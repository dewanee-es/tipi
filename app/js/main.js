...
// MOD
Content.markdown = function(val, md, callback) {
  'use strict';
  var original = new marked.Renderer();
  var renderer = new marked.Renderer();
  renderer.image = function(href, title, text) {
    if(href.indexOf(":") == -1) {
      href = val.url.substr(0, val.url.lastIndexOf("/") + 1) + href;
    }
    return original.image(href, title, text);
  };
  var options = {
...
Content.reloadPage = function(val) {
  ...
  Backend.loadContent(val.url, function(data) {
    Content.markdown(val, data, function(compiledMarkdown) {
