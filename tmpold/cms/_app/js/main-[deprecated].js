...
var configContent = {
  ...
  footer: {
	...
	poweredBy: 'Powered by <a href="http://morphus.net/tipi">Tipi</a>'
  },
  contacts: {},
  pages: {},
  posts: {},
...
Content.render = function(template, data) {
	...
	return Mark.up(template, data, {delimiter: ':'});
};
...
/******************* Initialization ****************/
Backend.init = function(appDir, contentJson) {
  contentJson.global.appDir = appDir;
  Loader.loadJavascript(appDir + '/js/jquery.js', function() {
	if(!Content.config(contentJson)) {
	  return;
	}
  ...
  //Loader.loadJavascript(appDir + '/js/twig.min.js');
  ...
  //Loader.loadJavascript(appDir + '/js/template7.min.js');
  ...
  (function() {
	...
	try {
		Loader.loadJson(filename, function(contentJson) {
			var appDir = contentJson.global.appDir || '_app';
			Loader.loadJavascript(appDir + '/js/main.js', function() {
				Backend.init(appDir, contentJson);
			});
		});
	} catch (e) {
		document.write('<strong>Error loading config</strong><br/>');
		if (e instanceof SyntaxError) {
			document.write('Config file syntax error: ' + filename + ' (' + e.message + ')');
		} else {
			document.write('Config file doesn\'t exist: ' + filename);
		}
	}
  })();